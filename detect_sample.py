...
def main(argv):
    ...
    with CytomineJob(options.cytomine_host, options.cytomine_public_key, options.cytomine_private_key,
                     options.cytomine_id_software, options.cytomine_id_project, verbose=str2bool(options.verbose)) as cj:
        ...
        for obj in image_instances:
            if True:
                print("Processing Cytomine imageid %d jobid %d" %(obj.id,job.userJob))
                cj.job.update(status=Job.RUNNING, progress=i/len(images)*100, statusComment="Processing images...")
                ...
                cv_im_thres = cv2.adaptiveThreshold(cv_im, 255, cv2.CV_ADAPTIVE_THRESH_GAUSSIAN_C,
                ...
                for geometry in locations:
                    ...
                    annotation = Annotation(geometry, obj.id).save()
                    print(annotation)
                    if annotation:
                        AnnotationTerm(annotation.id, parameters['cytomine_predict_term']).save()
                geometries = []


                i += 1

        cj.job.update(status=Job.TERMINATED, progress=100, statusComment="Finish Job..")
        print("END.")
